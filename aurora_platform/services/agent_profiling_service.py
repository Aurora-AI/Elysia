...
<<<<<<< HEAD
                        latencias = [
                            log_entry["latency"]
                            for log_entry in lista
                            if "latency" in log_entry
                        ]
                        custos = [
                            log_entry["token_cost"]
                            for log_entry in lista
                            if "token_cost" in log_entry
                        ]
                        success_count = sum(
                            1 for log_entry in lista if log_entry.get("success")
                        )
                        total = len(lista)
                        temp_usados = set(
                            str(log_entry.get("temperature"))
                            for log_entry in lista
                            if "temperature" in log_entry
=======
...
<<<<<<< HEAD
                        ...
                        if temp_usados:
                            report += (
                                f"- Temperaturas testadas: {', '.join(temp_usados)}\n"
                            )
=======
                            report += (

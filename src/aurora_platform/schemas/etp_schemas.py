...
class ETPRequest(BaseModel):
    ...
    tipo_obra: str = Field(..., description="Tipo da obra (ex: Construção, Reforma, Pavimentação)")
    ...
    documentos_referencia: Optional[List[str]] = Field(default=[], description="IDs dos documentos de referência")

class ETPResponse(BaseModel):
    """Schema para resposta da geração de ETP"""
    id: str = Field(..., description="ID único do ETP gerado")
    conteudo_markdown: str = Field(..., description="Conteúdo do ETP em formato Markdown")

...
from aurora_platform.modules.rag.models.rag_models import ChunkResult, DocumentMacro
...
def ensure_collections(cli: QdrantClient) -> None:
    if not cli.collection_exists(COLL_DOCS):
        cli.recreate_collection(
            COLL_DOCS, vectors_config=qm.VectorParams(size=DIM, distance=qm.Distance.COSINE)
        )
    if not cli.collection_exists(COLL_CHUNKS):
        cli.recreate_collection(
            COLL_CHUNKS, vectors_config=qm.VectorParams(size=DIM, distance=qm.Distance.COSINE)
        )

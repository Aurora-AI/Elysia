...
class LocalLLMService:
    ...
    def _load_pipeline(self):
        ...
        if self._summarizer is None:
            try:
                logger.info(f"Carregando modelo de sumarização local: {self.model_name}")
                self._summarizer = pipeline("summarization", model=self.model_name)
                logger.info("Modelo de sumarização carregado com sucesso.")
            except Exception as e:
                logger.exception(f"Falha ao carregar o pipeline do modelo '{self.model_name}': {e}")
                raise

    def summarize(self, text: str, max_length: int = 256, min_length: int = 64) -> str:
        """Gera um resumo do texto fornecido."""
        if not self._summarizer:
            raise RuntimeError("O pipeline de sumarização não foi inicializado corretamente.")

        try:
            # Limita o texto de entrada para evitar sobrecarga no modelo pequeno
            input_text = text[:8000] 
            result = self._summarizer(input_text, max_length=max_length, min_length=min_length, do_sample=False)

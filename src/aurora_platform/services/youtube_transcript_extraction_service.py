...
from aurora_platform.services.audio_transcription_service import AudioTranscriptionService
...
class YoutubeTranscriptExtractionService:
    ...
    def discover_video_urls(self, channel_url: str) -> List[str]:
        ...
        try:
            ...
            with open(output_path, "w", encoding="utf-8") as f:
                for url in urls:
                    f.write(url + "\n")
            logger.info(f"[OS-{self.ordem_servico_id}] {len(urls)} URLs salvas em {output_path}")
            return urls
    ...
    def extract_transcripts_from_channel(self, channel_url: str):
        ...
        summary = {"total_videos": len(video_urls), "success": 0, "fallback": 0, "failed": 0}
        for url in video_urls:
            video_id = url.split("v=")[-1] if "v=" in url else url.split("/")[-1]
            log_path = os.path.join(self.workspace_dir, f"transcript_{video_id}.log")
            transcript_txt_path = os.path.join(self.workspace_dir, f"transcript_{video_id}.txt")
            ...
            try:
                cmd = [
                    ...
                    "--convert-subs", "srt",
                    "-o", os.path.join(self.workspace_dir, "%(id)s.%(ext)s"),
                    url,
                ]
                logger.info(f"[OS-{self.ordem_servico_id}] Baixando legendas: {' '.join(cmd)}")
                ...
                if found:
                    with open(transcript_txt_path, "w", encoding="utf-8") as f:
                        f.write(transcript)
                    summary["success"] += 1
                    self._log(log_path, "success", url, "Transcrição extraída com sucesso.")
                else:
                    # Fallback para áudio
                    logger.warning(f"[OS-{self.ordem_servico_id}] Sem legendas em {url}, iniciando fallback por áudio.")
                    summary["fallback"] += 1
                    transcript = AudioTranscriptionService().transcribe_audio(url)
                    if transcript:
                        with open(transcript_txt_path, "w", encoding="utf-8") as f:
                            f.write(transcript)
                        self._log(log_path, "fallback", url, "Transcrição via áudio (fallback).")
                    else:
                        summary["failed"] += 1
                        self.fallback_report.append({"video_url": url, "status": "failed"})
                        self._log(log_path, "failed", url, "Transcrição falhou totalmente.")
        ...
        with open(os.path.join(self.workspace_dir, "fallback_report.json"), "w", encoding="utf-8") as f:
            json.dump(self.fallback_report, f, ensure_ascii=False, indent=2)
        with open(os.path.join(self.workspace_dir, "summary_report.json"), "w", encoding="utf-8") as f:

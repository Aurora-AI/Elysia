...
import { newTraceId, traceIdHeader, withTrace, logJSON } from "./observability.js";
...
function timeoutReject(ms: number, traceId: string) {
  return new Promise((_, rej) => setTimeout(() => rej(new Error(`PRIMARY_TIMEOUT_${ms}ms`)), ms));
}
...
async function handleLLM(prompt: string, traceId: string) {
  ...
  try {
    const result: any = await Promise.race([primary, timeoutReject(cfg.primaryTimeoutMs, traceId)]);
    if (isValid(result)) {
      logJSON(withTrace(traceId, { service: cfg.service.name, event: "primary_ok" }));
      return result.text!;
    }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    logJSON(withTrace(traceId, { service: cfg.service.name, event: "error", error: e?.message }));
...
server.listen(PORT, () => logJSON({ service: cfg.service.name, event: "listening", port: PORT }));

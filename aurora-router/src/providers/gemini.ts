...
export async function callGemini({ prompt, traceId }:
  { prompt: string; traceId: string; }): Promise<{ ok: boolean; text?: string; raw?: any }> {

  const body = {
    contents: [{ role: "user", parts: [{ text: `Complete apenas o trecho em falta: ${prompt}` }] }],
    generationConfig: {

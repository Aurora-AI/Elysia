...
st.set_page_config(
    page_title="Aurora Project Dashboard",
    page_icon="🚀",
    layout="wide"
)
...
def main():
    ...
    completed_tasks = sum(1 for epic in data['epics'] for task in epic['tasks'] if task['status'] == 'CONCLUÍDO')
    in_progress_tasks = sum(1 for epic in data['epics'] for task in epic['tasks'] if task['status'] == 'EM PROGRESSO')

...
async def autosave_session_to_memory(memory_service: Any, session_obj: Any) -> None:
    ...
    fn = getattr(memory_service, "add_session_to_memory",
                 None) or getattr(memory_service, "save", None)
    if fn is None:
        raise RuntimeError(
            "memory_service does not implement add_session_to_memory/save")
    res = fn(session_obj)
...
async def search_memory(memory_service: Any, app_name: str, user_id: str, query: str, team_id: str | None = None) -> list[dict[str, Any]]:
    ...
    fn = getattr(memory_service, "search_memory", None) or getattr(
        memory_service, "query", None)
    if fn is None:
        raise RuntimeError(
            "memory_service does not implement search_memory/query")
